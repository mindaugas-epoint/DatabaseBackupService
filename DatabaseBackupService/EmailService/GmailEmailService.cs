...
namespace DatabaseBackupService.EmailService
{
    public class GmailEmailService : IEmailService
    {
        ...
        public async Task SendBackupFailureNotificationAsync(List<(string databaseName, string error)> errors)
        {
            ...
            {
                _logger.WriteLog(Logger.LogLevel.Warning, "Email notification is not configured. Skipping email notification.");
                return;
            }
            ...
            _logger.WriteLog(Logger.LogLevel.Information, $"Backup failure notification email sent to {_emailConfig.RecipientEmail}");
        }
